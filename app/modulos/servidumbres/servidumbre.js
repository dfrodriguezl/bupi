...
const Form = ({ tbl, index, refresh, consecutivo }) => {
    ...
    useEffect(() => {
        ...
        var data = {
            "id_consulta": "info_header_form_servidumbre",
            "id_servidumbre": id,
        }
        ...
        if (index > 0) {
            ...
            servidorPost('/backend', data).then((response1) => {
                ...
                var data = {
                    "tabla": index,
                    "id_consulta": "get_formulario_servidumbre"
                }
    ...
    const crear_registro = () => {
        ...
        var data = {
            "tabla": "servidumbres." + tbl,
            "id_consulta": "insertar_vacio_servidumbres",
            "id_servidumbre": id
        }
...
const FormMultiple = ({ tbl, index, titulo }) => {
    ...
    useEffect(() => {
        ...
        if ([3, 7, 10, 11, 12, 13, 16, 19, 6, 8].includes(index)) {
            ...
            var data = {
                "id_consulta": "get_consecutivos_servidumbres",
                "id_servidumbre": id,
                "tabla": "servidumbres." + tbl
            }
    ...
    const add_registro = () => {
        ...
        var data = {
            "id_consulta": "insert_consecutivo_servidumbres",
            "id_servidumbre": id,
            "tabla": "servidumbres." + tbl
        }
...
const Ayuda = () => {
    ...
    return (
        ...
        <div className="header-help-form">
            <div><p>Número de matricula: </p>   <p>{info.matricula}</p></div>
...
const Servidumbre = () => {
    ...
    return (
        <div id="seccion">
            <div id="titulo_seccion">Servidumbre</div>
            ...
            <Tabs onSelect={() => setIndex(0)}>
                ...
                <TabPanel>
                    ...
                    <div className="grupo-formularios">
                        <button onClick={() => getForm(1, "info1_general_proyecto_servidumbre", "Información general del proyecto servidumbre")}
                            className={active == 1 ? 'active' : ''}    >
                            general del proyecto servidumbre
                        </button>
                        <button onClick={() => getForm(2, "info2_general_predio_sirviente", "Información general del predio sirviente")}
                            className={active == 2 ? 'active' : ''}    >
                            general del predio sirviente
                        </button>
                        <button onClick={() => getForm(3, "info3_general_predios_derivados", "Información general de predios derivados")}
                            className={active == 3 ? 'active' : ''}    >
                            general de predios derivados
                        </button>
                        <button onClick={() => getForm(4, "info4_areas_y_usos_predio_sirviente", "Áreas y usos predio sirviente")}
                            className={active == 4 ? 'active' : ''}    >
                            Áreas y usos predio sirviente
                        </button>
                        <button onClick={() => getForm(5, "info5_servidumbre", "Servidumbre")}
                            className={active == 5 ? 'active' : ''}    >
                            Servidumbre
                        </button>
                        <button onClick={() => getForm(6, "info6_avaluos_servidumbre", "Avaluos servidumbre")}
                            className={active == 6 ? 'active' : ''}    >
                            Avaluos servidumbre
                        </button>
                        <button onClick={() => getForm(7, "info7_orden_de_pago", "Orden de pago")}
                            className={active == 7 ? 'active' : ''}    >
                            Orden de pago
                        </button>
                        <button onClick={() => getForm(8, "info8_soportes_pago", "Soportes de pago")}
                            className={active == 8 ? 'active' : ''}    >
                            Soportes de pago
                        </button>
                        <button onClick={() => getForm(9, "info9_juridico_servidumbre", "Jurídico servidumbre")}
                            className={active == 9 ? 'active' : ''}    >
                            Jurídico servidumbre
                        </button>
                        <button onClick={() => getForm(10, "info10_cons_servidumbre", "Constitución servidumbre")}
                            className={active == 10 ? 'active' : ''}    >
                            Constitución servidumbre
                        </button>
                        <button onClick={() => getForm(11, "info11_propietario_catastral", "Propietario catastral")}
                            className={active == 11 ? 'active' : ''}    >
                            Propietario catastral
                        </button>
                        <button onClick={() => getForm(12, "info12_propietario_juridico_actual", "Propietario jurídico")}
                            className={active == 12 ? 'active' : ''}    >
                            Propietario jurídico
                        </button>
                        <button onClick={() => getForm(13, "info13_propietario_constitucion_servidumbre_juridico", "Propietario constitución servidumbre")}
                            className={active == 13 ? 'active' : ''}    >
                            Propietario constitución servidumbre
                        </button>
                        <button onClick={() => getForm(14, "info14_infraestructura_servidumbre", "Infraestructura servidumbre")}
                            className={active == 14 ? 'active' : ''}    >
                            Infraestructura servidumbre
                        </button>
                        <button onClick={() => getForm(15, "info15_saneamiento_basico_servidumbre", "Saneamiento básico servidumbre")}
                            className={active == 15 ? 'active' : ''}    >
                            Saneamiento básico servidumbre
                        </button>
                        <button onClick={() => getForm(16, "info16_municipios_intersectados_servidumbre", "Municipios intersectados servidumbre")}
                            className={active == 16 ? 'active' : ''}    >
                            Municipios intersectados servidumbre
                        </button>
                        <button onClick={() => getForm(17, "info17_documentos_requeridos_servidumbre", "Documentos requeridos servidumbre")}
                            className={active == 17 ? 'active' : ''}    >
                            Documentos requeridos servidumbre
                        </button>
                        <button onClick={() => getForm(18, "info18_control_calidad_tecnico_servidumbre", "Control calidad técnico")}
                            className={active == 18 ? 'active' : ''}    >
                            Control calidad técnico
                        </button>
