...
const Mapa = () => {
      ...
      data.forEach((d) => {
        if (d.tipo === 'geometria_verificada') {
          if (d.geojson.features != null) {
            ...
            toast.info("Servidumbre sin geometria");
          }
            ...
            cambioEstado.onclick = () => {
              ...
              servidorPost('/backend', dataPredio).then((response) => {
                if (response.data.length > 0) {
                  const id_exp_return = response.data[0].id_expedie;
  ...
  return (
    <div id="seccion">
      ...
      <p id="descripcion_seccion">En esta sección usted puede visualizar la información cartográfica de la servidumbre</p>
      <div id="visor1" >
      </div>
